    ...
    void run() throws CliException, IOException, InterruptedException {
        if (cl.getCommand() == null) {
            ...
            try {
                ...
                Object console = consoleC.getConstructor().newInstance();// 实例化命令行读取对象

                Object completor = completorC.getConstructor(ZooKeeper.class).newInstance(zk);// 实例化自定义自动填充对象
                Method addCompletor = consoleC.getMethod("addCompleter",
                        Class.forName("jline.console.completer.Completer"));
                addCompletor.invoke(console, completor);// 添加自定义补充对象
            ...
            if (jlinemissing) {// 如果JLine启动失败，则启动默认的console
